...
template class CBTree<char>;//模板类必须要做的一件事
...
template<class T>
void CBTree<T>::CreateBTree(char *a)
{
	CBTree<T> *s[80];//s数组作为存储二叉树中根结点指针的栈
	int top = -1;      //top作为s栈的栈顶指针
	left = NULL;       //先将left作为树根指针给予置空
	CBTree<T> *p = NULL;//定义p为指向二叉树结点的指针
...
//按任一种遍历次序输出二叉树中的所有结点
template<class T>
void CBTree<T>::TraverseBTree(int mark)
{
	Traverse(left, mark);
}
...
//用于遍历的递归函数
template<class T>
void CBTree<T>::Traverse(CBTree<T> *&BT, int mark)
{
	if (mark == 1) 
	{ //先序遍历
		if (BT != NULL)
...
//用于求二叉树深度的递归函数
template<class T>
int CBTree<T>::Depth(CBTree<T> *&BT)
{
	if (BT == NULL)
...
//用于求二叉树中所有结点数的递归函数
template<class T>
int CBTree<T>::Count(CBTree<T> *&BT)
{
	if (BT == NULL) 
		return 0;
	else
...
//用于求二叉树中所有叶子结点数的递归函数
template<class T>
int CBTree<T>::LeafCount(CBTree<T> *&BT)
{
	if (BT == NULL)
		return 0;
...
//用于输出整个二叉树的递归函数
template<class T>
void CBTree<T>::Print(CBTree<T> *&BT)
{
	if (BT == NULL) 
		return;//树为空时返回
	else {//否则执行如下操作
...
//用于清除二叉树的递归函数
template<class T>
void CBTree<T>::Clear(CBTree<T> *&BT)
{
	if (BT != NULL)
	{ //当二叉树非空时进行如下操作
		Clear(BT->left); //删除左子树
